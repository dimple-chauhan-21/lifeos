...
## Phase 3.4 - Entity Types Registry

**A genuine architectural ambiguity, flagged before implementing rather than guessed at**: `Database Architecture` §3 describes `entity_type` as a "controlled vocabulary" referencing a registry table, and consistently uses `entity_type` as a direct, human-readable string throughout the document (e.g., §18's composite index filters directly on it). But §15 states, unconditionally, "every table uses a UUID primary key... including entities and every detail and Platform table" — and `entity_types` is a Platform table. The document's own Quality Review only confirms the enum-vs-lookup-table choice was resolved, not this table's internal column shape. Presented both readings (string natural key vs. UUID surrogate + unique string column) for a decision before writing any code. **Decided: string natural key** — `entity_types.entity_type` (String, PK), matching how the column is used everywhere else in the document; §15's own rationale (avoiding ID-enumeration attacks on sensitive per-user resources) doesn't meaningfully apply to a small, fixed, non-sensitive vocabulary table.

Completed:
- `apps/api/app/platform/entities/models.py`: `EntityType` — exactly one column, `entity_type` (String, PK). No description, icon, color, or timestamp columns — none are named anywhere in the architecture for this table, and none were added speculatively
- `apps/api/alembic/versions/0002_create_entity_types.py`: creates `entity_types` only. No seed data — the architecture doesn't require any Domain types to be registered yet, since no Domain exists
- `app/platform/__init__.py` extended with the `EntityType` import, per the aggregator pattern adopted in 3.3 — worked correctly this time, no repeat of the empty-migration bug: autogenerate detected `entity_types` on the first attempt

Entity itself (the base `entities` table) was **not** built this phase — explicitly deferred to 3.5, per scope.

Hand-reviewed the generated migration against every point requested: primary key (`entity_type`, matching the confirmed design), constraints (only the primary key — no extras), nullable flags (`NOT NULL`, correct for a PK), downgrade correctness (`drop_table` fully reverses `create_table`, no data to worry about since nothing is seeded). No hand-editing of substance was needed — the autogenerated schema was already correct; only cosmetic cleanup (stripping Alembic's auto-generated comment markers, matching `0001`'s style) was applied.

Verified for real, against live Postgres in Docker:
- `alembic upgrade head` — `entity_types` created, confirmed via `\d entity_types`: exactly one column, `character varying`, `not null`, PK `entity_types_pkey` on `entity_type`
- **Constraints verified as actually enforced, not just declared**: a duplicate insert correctly rejected (`duplicate key value violates unique constraint`), a `NULL` insert correctly rejected (`null value ... violates not-null constraint`)
- `alembic downgrade -1` (one step back, not `base`) — `entity_types` genuinely removed while `users` (from `0001`) correctly persisted untouched, proving the migrations are properly incremental, not just an all-or-nothing reset
- `alembic upgrade head` again — `entity_types` recreated empty (correctly, no seed data expected), `users`' seeded row confirmed untouched throughout the whole cycle
- A fresh `--autogenerate` against the final state correctly detected zero diff, throwaway file deleted after
- Ruff, Ruff format, mypy (17 source files), and the full existing test suite (4/4) all clean, no regressions

Commit:
```
feat(api): add entity_types registry table

Add app/platform/entities/models.py: EntityType, a single-column
registry (entity_type: String, PK) implementing the "controlled
vocabulary" lookup table described in Database Architecture §3.
No seed data — no Domain exists yet to register. Entity itself
(the entities base table) is deferred to Phase 3.5.

String natural key chosen over a UUID surrogate after flagging a
real tension in the architecture doc: §15 says every table has a
UUID PK, but entity_type is used as a direct, human-readable string
everywhere else in the document (indexing, filtering). Decided with
explicit confirmation before implementing, given how foundational
this table is — every future entities row will reference it.

Verified end-to-end against live Postgres in Docker: PK and NOT NULL
constraints confirmed actually enforced (not just declared) via real
duplicate/null insert attempts; incremental downgrade (-1, not base)
